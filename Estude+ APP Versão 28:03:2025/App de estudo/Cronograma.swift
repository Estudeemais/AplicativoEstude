...
struct CriarCronograma: View {
    @Binding var cronogramas: [Cronograma] // Lista de cronogramas compartilhada
                    ...
                    Button(action: {
                        if !nomeCronograma.isEmpty {
                            let cronograma = Cronograma(
                                nome: nomeCronograma,
                                dataTermino: selectedDate,
                                diasDaSemana: Array(selectedDays),
                                materias: Array(selectedSubjects)
                            )
...
//                
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(action: {
                        if let url = URL(string: "https://drive.google.com/drive/folders/1u9OlXm8cCdGpvopTRoayphxrqnErjaaC?usp=sharing") {
                            openURL(url)
                        }
                    }) {
                        Text("Modelo")
